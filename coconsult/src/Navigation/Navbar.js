...
export default function NavbarDefault() {
  return (
    <Disclosure as="nav" className="bg-four rounded-2xl h-25">
      {({ open }) => (
        <>
                ...
                <Menu as="div" className="relative ml-3">
                  <div>
                    <Menu.Button className="relative flex rounded-full bg-one text-lg focus:outline-none focus:ring-2 focus:ring-one">
                      ...
                      <img
                        className="h-20 w-20 rounded-full bg-two"
                        src={logo}
                        alt="Profile"
                      />
                    </Menu.Button>
                  ...
                  >
                    <Menu.Items className="absolute right-0 z-10 mt-2 w-60 origin-top-right rounded-3xl bg-one py-1 shadow-lg ring-1 ring-white ring-opacity-5 focus:outline-none font-vango">
                      <Menu.Item>
                        {({ active }) => (
                          <a
                            href="/journal"
                            className={classNames(active ? 'bg-one rounded-full hover:text-white' : '', 'block px-5 py-5 text-lg text-white font-vango hover:text-white rounded-full')}
                          >
                           My Projects
                          </a>
    ...
    {navigation.map((item) => (
      <Disclosure.Button
        ...
        className={classNames(
          item.current ? 'bg-one text-white font-vango hover:text-white' : 'text-white hover:bg-one font-vango hover:text-white',
